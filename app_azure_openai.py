...
@st.cache_resource
def get_model_for_documents(MODEL_NAME):
...
search_upload = st.sidebar.radio("Select the Analysis Type", ('Search', 'Upload'))

if search_upload == 'Upload':
    ...
    if selected_category == 'Documents':
        ...
        if st.button('Submit'):
            ...
            pinecone.init(api_key=PINECONE_API_KEY, environment=PINECONE_REGION)
            index = pinecone.Index(PINECONE_INDEX_NAME)
            xc = index.query(embedding, top_k=5,
                            filter={"category": {"$eq": CATEGORY}},
                            include_metadata=True)
            contexts = [
                x['metadata']['filename'] for x in xc['matches']
            ]

            for i, context in enumerate(contexts):
                st.write(f"Top {i+1} result: {context}")
